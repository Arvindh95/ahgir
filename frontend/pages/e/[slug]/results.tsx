...
import { ArrowLeft, Check, CheckSquare, Download, Eye, Image as ImageIcon, Loader2, Share2, Link, X } from 'lucide-react'
...
export default function ScanResults() {
  ...
  const selectAll = () => {
    ...
    if (selectedIds.size === scanResult.matches.length) {
      setSelectedIds(new Set())
    } else {
      setSelectedIds(new Set(scanResult.matches.map(p => p.image_id)))
    }
  }
  ...
  const handleShare = async (photo: MatchedPhoto) => {
              ...
              <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 glass-card p-4 rounded-xl">
                <div className="flex items-center gap-4">
                  <button onClick={selectAll} className="flex items-center gap-2 text-sm text-gray-300 hover:text-white transition-colors">
                    ...
                    {selectedIds.size === scanResult.matches.length ? 'Deselect All' : 'Select All'}
                  </button>
                  {selectedIds.size > 0 && (
          ...
          >
            ...
            <div className="mt-6 flex items-center gap-4 bg-black/50 backdrop-blur-xl px-6 py-4 rounded-2xl border border-white/10">
              <div className="text-lg font-bold">
                 Match: <span className="text-green-400">{Math.round(selectedPhoto.similarity * 100)}%</span>
              </div>
