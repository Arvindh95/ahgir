...
async def delete_photo(
    ...
    try:
        ...
        events_logger.warning(f"Failed to delete MinIO objects for image {image_uuid}: {e}")
        ...
        for face in image_faces:
            if face.compreface_subject_id:
                try:
                    ...
                    events_logger.warning(f"Failed to delete CompreFace subject {face.compreface_subject_id}: {e}")
        ...
        for face in old_faces:
            if face.compreface_subject_id:
                try:
                    ...
                    events_logger.warning(f"Failed to delete CompreFace subject {face.compreface_subject_id}: {e}")
        events_logger.info(f"Cleaned {deleted_cf}/{len(old_faces)} CompreFace subjects for event {event_uuid}")
